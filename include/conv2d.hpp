...
public:
    ...
    // filters are height*width*depth. Images are depth*height*width.
    Image forward_pass(const Image& input) {
        ...
        Image tmp(this->last_input.size(), 
            Matrix(this->last_input[0].size() + 2*padding, 
                std::vector<Weight>(
                    this->last_input[0][0].size() + 2*padding,0
                )
            )
        );
        for(size_t d = 0; d < filters.size(); ++d) {
        ...
        */
        ...
        for(size_t out_depth = 0; 
            out_depth < this->filters.size(); 
            ++out_depth) 
        {
            ...
            for(size_t depth = 0; depth < this->input_depth; ++depth) {
                ...
                for(size_t row_front = 0, out_row = 0;
                    row_front < 
                        this->input_height - stride - 1;
                    row_front += stride, ++out_row) 
                {
                    for(size_t col_front = 0, out_col = 0;
                        col_front < 
                            this->input_width - stride - 1;
                        col_front += stride, ++out_col) 
                    {
                        // get window of the filter.
                        Matrix image_splice = aux::splice(image_at_depth,
                                                          row_front,
                                                          col_front,
                                                          filter_size);
                        Matrix window(filter_size, 
                                      std::vector<Weight>(filter_size));
                        // element wise product of values in window/filter.
                        ...
                        // add up all vales in the window.
                        double win_sum = 0;
                        for(auto& row : window) {
                            for(auto& val : row) {
                                win_sum += val;
                ...
                for(size_t i = 0; i < filter_size; ++i) {
                    for(size_t j = 0; j < filter_size; ++j) {
                        ...
                        Matrix sub_input(output_height,
                                std::vector<Weight>(output_width, 0)); 
                        for(size_t input_row = i, sub_row = 0; 
                            sub_row < output_height; 
                            input_row += stride, sub_row += 1) 
                        {
                            for(size_t input_col = j, sub_col = 0; 
                                sub_col < output_width;
                                input_col += stride, sub_col += 1) 
                            {
                                sub_input[sub_row][sub_col] = 
                                    this->last_input[cprime][input_row][input_col];
                            }
                        }
                        double window_sum = 0;
